# The Welsh Street Liquidity Engine
...
## 1. `provide-liquidity` - Standard AMM Behavior ✅
...
## 2. `lock-liquidity` - Permanent Locking (No LP Tokens) 🔒
...
**Tracking**: Increases `locked-a`, `locked-b`, and `locked-lp` variables
...
(define-public (lock-liquidity (amount-a uint))
  ...
  (begin
    ...
    (var-set locked-lp (+ (var-get locked-lp) amount-lp)) ;; Record tracking metric
    ;; NO LP tokens minted to user - liquidity becomes protocol-owned
```
...
## 3. `burn-liquidity` - LP Token Burning (Benefits Remaining LPs) 🔥
...
## 4. `remove-liquidity` - Withdraw Position (Subject to Tax) 📤
...
**Reward Reset**: User's reward tracking is updated when LP position changes
...
> **⚠️ Important**: Always claim your pending rewards before removing liquidity, as your reward position will be reset based on your new LP token balance.

---

## 5. `provide-initial-liquidity` - Admin Initial Setup (Fair Distribution) 🎯
...
> **⚠️ Critical Warning**: Always claim your pending rewards before changing your LP position (provide, remove, or burn). The `update-user-rewards` call will reset your reward tracking based on your new LP balance, potentially causing you to lose unclaimed rewards.
...
### `lock-liquidity` vs `burn-liquidity` - Different Purposes
...
### `provide-liquidity` vs `provide-initial-liquidity` - Setup vs Operation
...
## Parameter Limits (Security Features)
```clarity
(define-constant MAX_FEE u500)    ;; 5% maximum swap fees
(define-constant MAX_REV u500)    ;; 5% maximum revenue extraction
(define-constant MAX_TAX u2000)   ;; 20% maximum withdrawal tax
```

## Revenue Distribution
- **Swap Fees**: Go to rewards contract for LP holder distribution
- **Revenue Extraction**: Goes to protocol deployer (capped at 5%)
- **Withdrawal Tax**: Becomes permanently locked liquidity
...
This five-tier system **directly addresses each phase of the Welsh Economic Flywheel**, transforming WELSH from a static community token into the foundation of a dynamic, self-sustaining DeFi economy while ensuring mathematical fairness and community benefit alignment through the innovative initial distribution mechanism.

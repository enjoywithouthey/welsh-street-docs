...
(define-public (mint (amount uint))
    (begin
      ...
      (asserts! (is-eq contract-caller .exchange) ERR_NOT_CONTRACT_OWNER) ;; 🔒 CRITICAL SECURITY
...
```
WELSH + STREET → [Pool Contract] → Issues CREDIT Token (SIP-010)
                                        ↓
...
```
Multiple Tokens → [Central Pool Contract]
                         ↓

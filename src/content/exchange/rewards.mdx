...
# Welsh Street Exchange Rewards System

## Overview

The Welsh Street Exchange implements a block reset rewards strategy that provides fair, time-weighted reward distribution to liquidity providers while maintaining system simplicity and gas efficiency. This system eliminates whale manipulation vulnerabilities and provides a superior user experience compared to traditional AMM reward mechanisms.
...
### 3. Donations
- `donate-rewards` unlimited donation capability
- No artificial limits imposed because:
- Community Benefit: Donations benefit all LP holders proportionally
- Frontend access for direct treasury and governance distribution to LP holders.
...
The rewards system tracks each user's position and entry timing through a simple but powerful data structure:

```clarity
(define-map user-rewards
  { account: principal }
  {
    balance-lp: uint,      ;; Current LP token balance (real-time)
    block-lp: uint,        ;; Block when user last modified LP position (TIME TRACKING)
    debt-a: uint,          ;; Welsh rewards debt at entry (PREVENTS DILUTION)
    debt-b: uint,          ;; Street rewards debt at entry (PREVENTS DILUTION)
  }
)
```

Key Protection Mechanisms:
- `block-lp`: Time-weighted rewards - tracks when user entered position
- `debt-a/debt-b`: Anti-dilution protection - prevents claiming pre-entry rewards
- `balance-lp`: Real-time tracking - reflects current LP token holdings

### Claim Rewards Logic

The core claiming mechanism ensures fair distribution while preventing manipulation:

```clarity
(define-public (claim-rewards)
  (let (
    ;; Get current LP balance (can change without claiming)
    (balance-lp (unwrap! (contract-call? .credit get-balance tx-sender) ERR_LIQUIDITY_BALANCE_NOT_AVAILABLE))
    
    ;; Get user's historical entry data
    (info (default-to {balance-lp: u0, block-lp: u0, debt-a: u0, debt-b: u0}
                     (map-get? user-rewards { account: tx-sender })))
    
    ;; Calculate total accumulated rewards based on CURRENT LP balance
    (acc-a (/ (* balance-lp (var-get index-a)) DECIMALS))
    (acc-b (/ (* balance-lp (var-get index-b)) DECIMALS))
    
    ;; Get debt from when user entered position
    (deb-a (get debt-a info))
    (deb-b (get debt-b info))
    
    ;; Claimable = Total Accumulated - Entry Debt (TIME-WEIGHTED REWARDS)
    (claimed-a (if (> acc-a deb-a) (- acc-a deb-a) u0))
    (claimed-b (if (> acc-b deb-b) (- acc-b deb-b) u0))
  )
    ;; Transfer rewards WITHOUT affecting LP position
    (begin
      ...
      ;; PRESERVES block-lp (maintains time tracking)
      (map-set user-rewards { account: tx-sender } {
        balance-lp: balance-lp,
        block-lp: (get block-lp info),  ;; PRESERVED - no position change
        debt-a: acc-a,                  ;; UPDATED - prevents re-claiming
        debt-b: acc-b                   ;; UPDATED - prevents re-claiming
      })
...
How It Works:
1. Calculate total rewards user would get for their current LP balance
2. Subtract entry debt (rewards accumulated before they provided liquidity)
3. Transfer the difference (only rewards earned during their participation)
4. Update debt to prevent double-claiming
5. Preserve position tracking (`block-lp` unchanged - no position modification)

<Callout type="important">Position preservation only applies to `claim-rewards()`.
LP operations (provide/remove/lock/burn liquidity) call `update-user-rewards()` which RESETS everything.</Callout>
...
### Block Reset Strategy Eliminates Whale Attacks

Our `user-rewards` system automatically prevents whale manipulation. For example, if a whale deposits massive LP position:

- Day 30: Diva has 100K LP, accumulated 50,000 WELSH rewards
- Day 31: Whale deposits 10M LP
- Gets block-lp = current block (can't claim historical rewards)
- Gets debt-a/debt-b = current accumulated rewards (entry debt set)
- When claiming: Whale can't touch Diva's 50,000 WELSH (protected by debt mechanism)
- Result: Each user only gets rewards proportional to their time-weighted contribution

### Simple System vs Complex Anti-Whale Measures

The Welsh Street simple approach to reward tracking has many advantages, especially in the context of a blockchain.

✅ **Advantages of a Simple Debt Rewards System:**
- Gas Efficient: No complex time calculations or vesting logic
- Fair: Same rate for everyone regardless of position size
- Maintainable: Simple code is more secure and auditable
...
|--------|----------------------|-------------------|
| Gas Costs | ✅ Minimal | ❌ High (time calculations) |
| User Experience | ✅ Predictable | ❌ Confusing penalties |
| Security | ✅ Simple = secure | ❌ Complex = attack surface |
| Whale Protection | ✅ Automatic via debt | ❌ Artificial restrictions |
| Maintenance | ✅ Easy to audit | ❌ Complex edge cases |
| Time Lockups | ✅ None | ❌ Annoying |
...
### Welsh Street's Superior Approach
...
→ User loses money to claim rewards → Discourages reward claiming
```
...
→ IL stays UNREALIZED → Can recover if prices revert → Win-win scenario
```
...
❌ Net result: $25 rewards - $16 IL = $9 gain 😰
```
...
4. update-user-rewards resets block-lp and debt to current values
5. Unclaimed rewards are LOST FOREVER ❌
6. User's new LP position starts fresh with zero debt
```
...
(define-public (update-user-rewards (user principal))
  (let (
    (current-lp-balance (unwrap! (contract-call? .credit get-balance user) ERR_LIQUIDITY_BALANCE_NOT_AVAILABLE))
    (current-index-a (var-get index-a))
    (current-index-b (var-get index-b))
    ...
    (begin
      ...
      ;; RESET: Set new baseline for reward calculations
      (map-set user-rewards { account: user } {
        balance-lp: current-lp-balance,  ;; Current LP balance
        block-lp: current-block,         ;; NEW entry time
        debt-a: (/ (* current-lp-balance current-index-a) DECIMALS),  ;; NEW debt baseline
        debt-b: (/ (* current-lp-balance current-index-b) DECIMALS),  ;; NEW debt baseline
      })
      (ok true)
    )
  )
